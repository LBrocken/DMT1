...
Answer here: It's wrong when y already has an
existing definition outside of the function.
Renaming x to y would cause the function to
refer to the new parameter instead of the
external variable, which would change the
function's logic.
@@@ -/
...
Answer here: The type of the function is "Type → Type",
meaning it's a function that maps a type to a type.
@@@ -/
...
Answer: The type of the function is "(α : Type) →
α → α", meaning it takes a Type, called α, takes a
value of type α, and finally returns a value of type α.
@@@ -/
...
Answer here: The type of the idPoly' function is
"(α : Type) → α → α", just like the function attributed
to question #10.
@@@ -/

def idPoly' {α : Type} (a : α) : α := a
...
def ifThenElsePoly {α : Type} (b : Bool) (t : α) (e : α) :=
  if b then t else e
